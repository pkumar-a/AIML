...
def markdoctors(text, trie, doctors):
...
def getattributes1(uinput,context,attributes):
    ...
    if context.name.startswith('IntentComplete'):
        ...
        if context.name=='FirstGreeting':
            ...
            if names!=[]:
                attributes['PatientName'] = names[0]

            # print matches,"after checking names"
            doctors_trie, doctors_dict = loadDoctors('./entities/doctors.dat')
            attributes,matches = getDoctor(uinput,doctors_trie,doctors_dict,context,attributes,matches)
            ...
            for match in matches['Name']:
                uinput = re.sub(match,r'$patientName',uinput,flags=re.IGNORECASE)

            for match in matches['Date']:
                uinput = re.sub(match,r'$date',uinput,flags=re.IGNORECASE)

            for match in matches['Doctor']:
                uinput = re.sub(match,r'$doctor',uinput,flags=re.IGNORECASE)
        ...
        if '_'.join(context.name.split('_')[2:])=='PatientName':
            names,matches = getNames(uinput, context,matches)   
            if names!=[]:
                attributes['PatientName'] = names[0]
